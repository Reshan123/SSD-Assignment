...
const getallRequestForms = async (req, res) => {
  const requestForms = await adoptionRequestForm
    .find({})
    .sort({ createdAt: -1 });
...
const getRequestForm = async (req, res) => {
  ...
  try {
    ...
    logger.error("Adoption request controller error", {
      error: error.message,
      operation: "getSingleAdoptionRequest",
      id: req.params.id,
    });
...
const updateRequestForm = async (req, res) => {
  ...
  const form = await adoptionRequestForm.findOneAndUpdate(
    { _id: id },
    {
      ...req.body,
    }
